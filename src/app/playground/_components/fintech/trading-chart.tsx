...
export default function TradingChart() {
  ...
  useEffect(() => {
    ...
    const drawChart = () => {
      ...
      data.forEach((candle, i) => {
        ...
        ctx.fillRect(
          x,
          canvas.height - padding.bottom - volumeHeight,
          candleWidth - 1,
          volumeHeight
        );
      });
      ...
      const timeFormat =
        interval >= 1440 ? "MMM dd" : interval >= 60 ? "HH:mm" : "HH:mm:ss";
      for (
        let i = 0;
        i < data.length;
      ...
      if (selectedCandle) {
        ...
        const y =
          chartHeight -
          ((selectedCandle.close - minPrice) / priceRange) * chartHeight +
          padding.top;
        ctx.moveTo(padding.left, y);
        ctx.lineTo(canvas.width - padding.right, y);
        ctx.stroke();
  ...
  const CandleInfo = ({
    ...
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className={`absolute top-4 left-4 bg-gray-800 border border-gray-700 p-3 rounded-lg shadow-lg ${
        isSelected ? "border-blue-500" : ""
      }`}
    >
      <p className="text-gray-200 font-bold mb-1">
        {format(new Date(candle.time * 1000), "MMMM dd, yyyy HH:mm")}
      </p>
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-4 shadow-lg">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
        <div className="text-2xl font-bold text-gray-100 mb-2 sm:mb-0">
          BTC/USDT
        </div>
      ...
      <div className="relative">
        ...
        />
        <AnimatePresence>
          {isMouseOnChart && hoveredCandle && (
            <CandleInfo candle={hoveredCandle} />
          )}
          {!isMouseOnChart && selectedCandle && (
            <CandleInfo candle={selectedCandle} isSelected={true} />
          )}
        </AnimatePresence>
      </div>
    </div>
